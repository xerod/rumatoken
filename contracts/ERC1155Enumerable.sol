...
abstract contract ERC1155Enumerable is ERC1155, IERC1155Enumerable {
    ...
    mapping(uint256 => EnumerableSet.AddressSet) private _ownedTokens;

    /**
     * @dev Returns the address who own different tokenIds stored by the contract.
     */
    function getAllTokenOwner(uint256 _tokenId)
        public
        view
        virtual
        returns (address[] memory owner)
    {
        uint256 tokenIdOwner = _ownedTokens[_tokenId].length();
        owner = new address[](tokenIdOwner);

        // put owner address to array
        for (uint256 i = 0; i < _ownedTokens[_tokenId].length(); i++) {
            owner[i] = (_ownedTokens[_tokenId].at(i));
        }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (from == address(0)) {
                ...
                // all transferred
                _ownedTokens[tokenId].remove(from);
            }
